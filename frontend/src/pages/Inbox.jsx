...
export default function Inbox({ legalMode = false, instanceList = null, onOpenLegalClient, onOpenLegalDocuments }) {
  ...
  async function handleReturnToBot() {
    toast.confirm('Devolver este atendimento para o robô de IA?', async () => {
      try {
        await updateTicket(selectedId, { status: 'bot' });
        setSelectedId(null);
        loadTickets();
        toast.success('Atendimento devolvido para a IA!');
      } catch (e) { toast.error('Erro ao devolver: ' + (e.response?.data?.error || e.message)); }
    });
  }
...
export const inboxStyles = {
  ...
  tabsWrap: { padding: '0.75rem 0.6rem 0.7rem', borderBottom: '1px solid var(--border-color)' },
  tabs: { display: 'flex', gap: '2px', background: 'var(--bg-panel)', padding: '3px', borderRadius: 'var(--radius-md)', minWidth: 0 },
  tab: { 
    flex: 1, 
    minWidth: 0,
